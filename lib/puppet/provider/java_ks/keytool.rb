...
Puppet::Type.type(:java_ks).provide(:keytool) do
  ...
  def import_ks
    ...
    if @resource[:password_file].nil?
      raise Puppet::Error "You must pass either 'password' or 'password_file'." if @resource[:password].nil?
      tmpfile = Tempfile.new("#{@resource[:name]}.")
      ...
      raise Puppet::Error "You must pass either 'password' or 'password_file'." unless @resource[:password].nil?
